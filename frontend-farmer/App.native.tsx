        ...
        <ScrollView contentContainerStyle={{ padding: 20 }}>
          ...
          <View style={styles.authHeaderBanner}>
            <View style={{ alignItems: 'center', marginBottom: 12 }}>
              <View style={{ width: 84, height: 84, borderRadius: 42, backgroundColor: '#ffffff', alignItems: 'center', justifyContent: 'center', borderWidth: 3, borderColor: '#15803d', shadowColor: '#15803d', shadowOpacity: 0.3, shadowRadius: 8, elevation: 6 }}>
          ...
          <View style={styles.authCard}>
            ...
            {/* OPERATOR REDIRECT NOTICE FOR MOBILE */}
            {userRole === 'operator' && (
              <View style={{ alignItems: 'center', paddingVertical: 10 }}>
                <Text style={styles.authCardTitle}>Mandi Operator Desk Access</Text>
                <Text style={[styles.authCardSub, { color: '#0369a1', marginTop: 8 }]}>
                  Mandi Operator tools (QR Scanning, Lab Assaying & Electronic Weighbridge) are restricted to Desktop Web Desks for security & RBAC compliance.
                </Text>
                <View style={{ backgroundColor: '#f0f9ff', borderColor: '#bae6fd', borderWidth: 1, borderRadius: 10, padding: 14, marginVertical: 14, width: '100%' }}>
                  <Text style={{ fontSize: 13, fontWeight: 'bold', color: '#0369a1', marginBottom: 4 }}>Web Access Required:</Text>
                  <Text style={{ fontSize: 12, color: '#334155' }}>Please open http://{getNativeHost()}:3010 (or http://localhost:3010) on your Mandi Workstation Web Browser.</Text>
                </View>
                <TouchableOpacity style={[styles.primaryBtn, { backgroundColor: '#0284c7' }]} onPress={() => Linking.openURL(`http://${getNativeHost()}:3010`)}>
                  <Text style={styles.primaryBtnText}>Open Operator Web Portal</Text>
                </TouchableOpacity>
                <TouchableOpacity style={{ marginTop: 12 }} onPress={() => setUserRole('farmer')}>
                  <Text style={{ color: '#15803d', fontWeight: 'bold', fontSize: 13 }}>Return to Farmer Mobile App</Text>
                </TouchableOpacity>
              </View>
            )}

            {/* ADMIN REDIRECT NOTICE FOR MOBILE */}
            {userRole === 'admin' && (
              <View style={{ alignItems: 'center', paddingVertical: 10 }}>
                <Text style={styles.authCardTitle}>DoCA Central Admin Governance</Text>
                <Text style={[styles.authCardSub, { color: '#273b64', marginTop: 8 }]}>
                  Government Governance tools (Centre Capacity, Operator Credentials & National Analytics) require Desktop Web access for security & RBAC compliance.
                </Text>
                <View style={{ backgroundColor: '#f8fafc', borderColor: '#cbd5e1', borderWidth: 1, borderRadius: 10, padding: 14, marginVertical: 14, width: '100%' }}>
                  <Text style={{ fontSize: 13, fontWeight: 'bold', color: '#273b64', marginBottom: 4 }}>Web Access Required:</Text>
                  <Text style={{ fontSize: 12, color: '#334155' }}>Please open http://{getNativeHost()}:3020 (or http://localhost:3020) on your Governance Desktop Web Browser.</Text>
                </View>
                <TouchableOpacity style={[styles.primaryBtn, { backgroundColor: '#273b64' }]} onPress={() => Linking.openURL(`http://${getNativeHost()}:3020`)}>
                  <Text style={styles.primaryBtnText}>Open Admin Web Portal</Text>
                </TouchableOpacity>
                <TouchableOpacity style={{ marginTop: 12 }} onPress={() => setUserRole('farmer')}>
                  <Text style={{ color: '#15803d', fontWeight: 'bold', fontSize: 13 }}>Return to Farmer Mobile App</Text>
                </TouchableOpacity>
    ...
    <SafeAreaView style={styles.container}>
      ...
      <View style={styles.topMasterBar}>
        <View style={{ flexDirection: 'row', alignItems: 'center', gap: 8 }}>
          <View>
            <Text style={{ fontSize: 14, fontWeight: 'bold', color: '#15803d' }}>SmartProcure Kisan Mobile App</Text>
            <Text style={{ fontSize: 10, color: '#64748b' }}>Dept. of Consumer Affairs (DoCA)</Text>
          </View>
        </View>

        <TouchableOpacity style={styles.signOutBtnMini} onPress={() => setIsAuthenticated(false)}>
          <Text style={{ fontSize: 11, color: '#ef4444', fontWeight: 'bold' }}>Sign Out</Text>
        </TouchableOpacity>
      </View>
