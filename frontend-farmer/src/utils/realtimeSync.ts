...
class RealtimeSyncManager {
  ...
  private portalName: 'farmer' | 'operator' | 'admin' = 'farmer';
  ...
  private initSocket() {
    try {
      ...
      // Bind all standard server events
      ...
      serverEvents.forEach((evt) => {
        this.socket?.on(evt, (payload: any) => {
          ...
          // Relay to broadcast channel so other tabs on this machine stay in sync
  ...
  public broadcast(eventType: SyncEventType, payload: any) {
    ...
    // 4. Emit to backend socket if connected
